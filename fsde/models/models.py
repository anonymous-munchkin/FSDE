...
default_params = dict(kernel='Matern32',
                      k_var=1.,
                      lengthscale=1.,
                      var=0.1,
                      jitter=1e-8,
...
class FSDE_SVI:
    ...
    def __init__(
        ...
        P: int, 
        M: int, 
        train_times: Union[f64['T'], f64['T 1']], 
        params: Optional[dict] = default_params
    ):
        super().__init__()

        self.kernel = params['kernel']
        self.nu = get_state_dim(self.kernel) # Matern smoothness
        self.L = L # Number of latent SDEs
        self.P = P # Number of outputs
        self.M = M # Number of inducing points for each latent SDE
        self.T = train_times.size # Number of time points in the training data (used for ELBO scaling)
        self.D = get_state_dim(self.kernel)
        self.key = params['key']
        self.jitter = params['jitter']
        
        # Initialize (unconstrained) model parameters
        self.key, subkey = jr.split(self.key)
        self.W = jr.normal(subkey, shape=(self.P,self.L)) # Mixing matrix (Random entries)
        self.unc_lengthscales = inv_softplus(params['lengthscale'] * jnp.ones((self.L,1))) # Unconstrained kernel lengthscale
        self.unc_k_vars = inv_softplus(params['k_var'] * jnp.ones((self.L,1))) # Unconstrained kernel signal variance
        self.unc_var = inv_softplus(params['var']) # Unconstrained observation noise 

        # Initialize variational parameters
        self.key, subkey = jr.split(self.key)
        self.ind_times = self.set_ind_times(self.M, train_times, mode='equal') # Inducing times [M,]
        self.first_ind_time = self.ind_times[0]
        self.last_ind_time = self.ind_times[-1]
        self.v_mean = jr.normal(subkey, (self.L,self.D,self.M)) # Variational mean [L,D,M]
        self.unc_R_diag_blocks, self.unc_R_offdiag_blocks = self.init_R(self.L, self.M, self.D) # Variational precision [L,M,D(D+1)/2], [L,M-1,D,D]

        # Partialize functions for concretization
        if self.kernel == 'Matern32':
            self.compute_cov_infty = cov_infty_Matern32
            self.get_sde_params = sde_params_Matern32
            self.compute_F = compute_F_Matern32

        elif self.kernel == 'Matern52':
            self.compute_cov_infty = cov_infty_Matern52
            self.get_sde_params = sde_params_Matern52
            self.compute_F = compute_F_Matern52

    def __repr__(self) -> str:
        return 'Factorial SDE with Sparse Variational Inference (FSDE-SVI)'

    def get_MOGP_params(self) -> dict:
        """Returns the MOGP parameter values."""

        MOGP_params = dict(W=self.W,
                           unc_lengthscales=self.unc_lengthscales,
                           unc_k_vars=self.unc_k_vars,
                           unc_var=self.unc_var)
    ...
    def get_v_params(self) -> dict:
        ...
        v_params = dict(ind_times=self.ind_times, # [M,]
                        v_mean=self.v_mean, # [L,D,M]
                        unc_R_diag_blocks=self.unc_R_diag_blocks, # [L,M,D(D+1)/2]
                        unc_R_offdiag_blocks=self.unc_R_offdiag_blocks) # [L,M-1,D,D]

        return v_params

    def set_ind_times(self, M, train_times, mode='K-means', margin=0.15):
    ...
    def precompute_pred_args(self):
        ...
        # Compute state-transition matrix
        F = vmap(self.compute_F, in_axes=0)(self.unc_lengthscales)

        # Compute steady-state covariance
        cov_infty = self.compute_cov_infty(self.unc_k_vars, self.unc_lengthscales) # [L,D,D]

        # Constrain R
        R_diag_blocks, R_offdiag_blocks = vmap(constrain_R, in_axes=(0,0))(self.unc_R_diag_blocks,
                                                                           self.unc_R_offdiag_blocks)

        # Compute S from R
        # Note: S_diag_blocks [L,M,D,D], S_offdiag_blocks [L,M-1,D,D]
        S_diag_blocks, S_offdiag_blocks = vmap(self.R_to_S, in_axes=(0,0))(R_diag_blocks, R_offdiag_blocks)
        
        # Compute inverse blocks of S
        # Note: S_inv_diag_blocks [L,M,D,D], S_offdiag_blocks [L,M-1,D,D]
        S_inv_diag_blocks, S_inv_offdiag_blocks = vmap(self.tridiag_inv, in_axes=(0,0))(S_diag_blocks, S_offdiag_blocks)

        return F, self.unc_var, self.W, cov_infty, self.v_mean, S_inv_diag_blocks, S_inv_offdiag_blocks 
    ...
    def init_R(
        ...
        triangular = tfb.FillTriangular(upper=False)
        unc_R_diag_blocks = vmap(triangular.inverse, in_axes=0)(unc_R_diag_blocks) # [L,M,D*(D+1)/2]
    ...
    def S_to_R(
        ...
        # Initial diagonal Cholesky block
        R_i_diag_block_1 = jscipy.linalg.cholesky(S_i_diag_blocks[0,:,:], lower=True) # [D,D]
        ...
        def helper(R_i_diag_prev, idx):
            ...
            # Find \bmR_{\ell,i}
            res = S_i_diag_block - R_i_offdiag_block @ R_i_offdiag_block.T
            R_i_diag_block = jscipy.linalg.cholesky(res, lower=True)
    ...
    def params_to_nat(
        ...
        # Constrain R
        R_diag_blocks, R_offdiag_blocks = vmap(constrain_R, in_axes=(0,0))(unc_R_diag_blocks,
                                                                           unc_R_offdiag_blocks)
        
        # Compute S
        S_diag_blocks, S_offdiag_blocks = vmap(self.R_to_S, in_axes=(0,0))(R_diag_blocks, # [L,M,D,D]
                                                                           R_offdiag_blocks) # [L,M-1,D,D]
    ...
    def nat_to_params(
        ...
        # Compute unconstrained log-Cholesky blocks
        unc_R_diag_blocks, unc_R_offdiag_blocks = vmap(self.S_to_R, in_axes=(0,0))(S_diag_blocks, 
                                                                                   S_offdiag_blocks)

        # Solve block tridiagonal system to find the variational mean
        v_mean = vmap(self.solve_tridiag_system, in_axes=(0,0,0))(eta_1, S_diag_blocks, S_offdiag_blocks) # [L,D,M]
    ...
    def params_to_exp(
        ...
        # Compute outer product of mean
        outer_diag_blocks = vmap(vmap(jnp.outer, in_axes=(1,1)), in_axes=(0,0))(v_mean, v_mean) # [L,M,D,D]
        outer_offdiag_blocks = vmap(vmap(jnp.outer, in_axes=(1,1)), in_axes=(0,0))(v_mean[:,:,1:],
                                                                                   v_mean[:,:,:-1]) # [L,M-1,D,D]
    ...
    def exp_to_params(
        ...
        # Compute outer product of mean
        outer_diag_blocks = vmap(vmap(jnp.outer, in_axes=(1,1)), in_axes=(0,0))(xi_1, xi_1) # [L,M,D,D]
        outer_offdiag_blocks = vmap(vmap(jnp.outer, in_axes=(1,1)), in_axes=(0,0))(xi_1[:,:,1:],
                                                                                   xi_1[:,:,:-1]) # [L,M-1,D,D]
        ...
        def compute_diag_blocks(S_i_inv_diag_blocks, S_i_inv_offdiag_blocks):
            diag_final = S_i_inv_diag_blocks[-1,:,:] # [D,D]
            R_i_diag_final = jscipy.linalg.cholesky(jnp.linalg.inv(diag_final + self.jitter * jnp.eye(self.D)),
                                                    lower=True) # [D,D]
            ...
            def diag_helper(prev, idx):
                ...
                schur = S_i_inv_diag_current - quadratic(S_i_inv_offdiag, S_i_inv_diag_next)
                schur_inverse = jnp.linalg.inv(schur + self.jitter * jnp.eye(self.D))
                R_i_diag_block = jscipy.linalg.cholesky(schur_inverse, lower=True)
        ...
        def compute_offdiag_blocks(S_i_inv_diag_blocks, S_i_inv_offdiag_blocks, R_i_diag_blocks):
            ...
            def offdiag_helper(prev, idx):
                ...
                R_i_offdiag_block = -inv_product(S_i_inv_offdiag, S_i_inv_diag) @ R_i_diag # [D,D]
        ...
        R_diag_blocks = vmap(compute_diag_blocks, in_axes=(0,0))(S_inv_diag_blocks, S_inv_offdiag_blocks) # [L,M,D,D]
        R_offdiag_blocks = vmap(compute_offdiag_blocks, in_axes=(0,0,0))(S_inv_diag_blocks,
                                                                         S_inv_offdiag_blocks,
                                                                         R_diag_blocks) # [L,M-1,D,D]

        # Unconstrain R
        unc_R_diag_blocks, unc_R_offdiag_blocks = vmap(unconstrain_R, in_axes=(0,0))(R_diag_blocks,
                                                                                     R_offdiag_blocks)
    ...
    def transition(
        ...
        def transition_helper(Z_prev, current):
            ...
            # Compute mean
            psi_k = vmap(compute_psi, in_axes=(None,0))(time_diff, F) # [L,D,D]
            Z_k_mean = jnp.einsum('ijk,ik->ij', psi_k, Z_prev) # [L,D,D], [L,D] -> [L,D]

            # Compute transition covariance
            phi_k = vmap(cov_steady, in_axes=(None,0,0))(time_diff, F, cov_infty) # [L,D,D]
            
            # Compute Cholesky decomposition for phi_k for more efficient sampling
            chol_k = vmap(jscipy.linalg.cholesky, in_axes=(0,None))(phi_k, True) # [L,D,D]
    ...
    def emission(
        ...
        Y_mean = self.W @ Z[:,0,:] # [P,L] * [L,T] -> [P,T]
        
        _, subkey = jr.split(key)
        std_normal = jnp.sqrt(softplus(self.unc_var)) * jr.normal(subkey, Y_mean.shape)
        Y = Y_mean + std_normal
    ...
    def sample(
        ...
        # Sample initial state from steady-state covariance
        cov_infty = self.compute_cov_infty(self.unc_k_vars, self.unc_lengthscales) # [L,D,D]
        chol_cov_infty = vmap(jscipy.linalg.cholesky, in_axes=(0,None))(cov_infty, True) # [L,D,D]
    ...
    def compute_Lambda(
        ...
        # Compute diagonal blocks
        T1 = vmap(inv, in_axes=(0,None))(ind_phi_i, self.jitter) # [M-1,D,D]
        T2 = vmap(quadratic, in_axes=(0,0))(ind_psi_i, ind_phi_i) # [M-1,D,D]
        ...
        # Compute lower off-diagonal blocks
        Lambda_offdiag_blocks = -vmap(inv_product, in_axes=(0,0))(ind_psi_i, ind_phi_i) # [M-1,D,D]
    ...
    def tridiag_inv(
        ...
        A = jnp.vstack((A, A_T[None,:])) # [M,D,D]
        
        B_1 = jnp.zeros((self.D,self.D))
        _, B = lax.scan(schur_helper_B, B_1, jnp.arange(1,self.M)) # B = [M-1,D,D], index starts at 1 
        B = jnp.vstack((B_1[None,:], B)) # [M,D,D]
        
        # Compute main diagonal blocks
        inv_diag_blocks = vmap(jnp.linalg.inv, in_axes=0)(diag_blocks - A - B + self.jitter * jnp.eye(self.D)) # [M,D,D]
        
        # Compute lower off-diagonal blocks
        inv_offdiag_blocks = vmap(offdiag_helper, in_axes=(0,0,0,0))(diag_blocks[1:], 
                                                                     A[1:], 
                                                                     offdiag_blocks, 
                                                                     inv_diag_blocks[:-1]) # [M-1,D,D]
    ...
    def tridiag_logdet(
        ...
        def det_helper(A_prev, k, blocks=blocks):
            ...
            A_k = lax.cond(cholesky, chol_f, inv_f)
            logdet_k = jnp.linalg.slogdet(A_k)[-1]
            
            return A_k, logdet_k
            
        _, logdets = lax.scan(det_helper, diag_blocks[0], jnp.arange(1,self.M))
        logdet = jnp.linalg.slogdet(diag_blocks[0])[-1] +  jnp.sum(logdets)
    ...
    def KL(
        ...
        def m_Lambda_m(
            ...
            result = jnp.sum(vmap(helper, in_axes=(1,0,1))(v_mean_i, Lambda_i_diag_blocks, v_mean_i))
            result += 2 * jnp.sum(vmap(helper, in_axes=(1,0,1))(v_mean_i[:,1:],
                                                                Lambda_i_offdiag_blocks,
                                                                v_mean_i[:,:-1]))
            return result
        
        def trace_Lambda_S_inv(
            Lambda_i_diag_blocks: f64['M D D'], 
            Lambda_i_offdiag_blocks: f64['M-1 D D'],
            S_i_inv_diag_blocks: f64['M D D'], 
            S_i_inv_offdiag_blocks: f64['M-1 D D']
            ...
            tmp_1 = jnp.einsum('ijk,ikl->ijl', Lambda_i_diag_blocks, S_i_inv_diag_blocks) # [M,D,D]
            trace_1 = jnp.sum(jnp.trace(tmp_1, axis1=1, axis2=2))
            
            tmp_2 = jnp.einsum('ijk,ilk->ijl', Lambda_i_offdiag_blocks, S_i_inv_offdiag_blocks) # [M-1,D,D]
            trace_2 = jnp.sum(jnp.trace(tmp_2, axis1=1, axis2=2))
        ...
        triangular = tfb.FillTriangular(upper=False)
        R_i_diag_blocks = triangular.forward(R_i_diag_blocks) # [M,D,D]
        logdet_S_i = 2 * jnp.sum(jnp.log(vmap(jnp.diag, in_axes=0)(R_i_diag_blocks)))
        
        kl = -0.5 * logdet_S_i
        kl += 0.5 * self.tridiag_logdet(Lambda_i_diag_blocks, Lambda_i_offdiag_blocks)
        kl += 0.5 * self.M * self.D
        kl += -0.5 * m_Lambda_m(v_mean_i, Lambda_i_diag_blocks, Lambda_i_offdiag_blocks)
        kl += -0.5 * trace_Lambda_S_inv(Lambda_i_diag_blocks, Lambda_i_offdiag_blocks,
                                        S_i_inv_diag_blocks, S_i_inv_offdiag_blocks)
        
        return kl

    def compute_ELBO(self, params: dict, batch: "Dataset") -> f64:
        """Computes the ELBO given a batch of data."""

        # Unpack parameters
        W = params['W']
        unc_var = params['unc_var']
        unc_lengthscales = params['unc_lengthscales']
        unc_k_vars = params['unc_k_vars']
        v_mean = params['v_mean']
        unc_R_diag_blocks = params['unc_R_diag_blocks']
        unc_R_offdiag_blocks = params['unc_R_offdiag_blocks']
        ...
        # Compute steady-state covariance
        cov_infty = self.compute_cov_infty(unc_k_vars, unc_lengthscales) # [L,D,D]
        ...
        # Compute state-transition matrix
        F = vmap(self.compute_F, in_axes=0)(unc_lengthscales)
        
        # Compute psi and phi
        ind_prior_psi = vmap(vmap(compute_psi, in_axes=(0,None)), 
                             in_axes=(None,0))(ind_time_diffs, F) # [L,M-1,D,D]
        ind_prior_phi = vmap(vmap(cov_steady, in_axes=(0,None,None)),
                             in_axes=(None,0,0))(ind_time_diffs, F, cov_infty) # [L,M-1,D,D]

        # Computes the precision and covariance blocks for all latent SDEs
        # Note: Lambda_diag_blocks: [L,M,D,D], Lambda_offdiag_blocks: [L,M-1,D,D]
        Lambda_diag_blocks, Lambda_offdiag_blocks = vmap(self.compute_Lambda, in_axes=(0,0,0))(ind_prior_psi, 
                                                                                               ind_prior_phi, 
                                                                                               cov_infty)
        ...
        # Get predictions
        approx_mean, approx_cov = self.predict(times_batch, F, unc_var, W, cov_infty, v_mean,
                                               S_inv_diag_blocks, S_inv_offdiag_blocks, output_latent=True)
        
        Y_hat = W @ approx_mean[:,:,0] # [P,L] x [L,V] -> [P,V]
        var = softplus(unc_var)
        
        # Expected log-likelihood
        exp_loglik = -0.5 * batch_size * self.P * jnp.log(2 * jnp.pi * var)
        exp_loglik += -0.5 * (1./var) * jnp.linalg.norm(Y_batch - Y_hat)**2
        exp_loglik += -0.5 * (1./var) * jnp.sum(vmap(trace_W_Cov, in_axes=(1,0), out_axes=0)(W, S_inv_diag_blocks))
        exp_loglik *= (self.T / batch_size) 

        # KL terms
        kl = jnp.sum(vmap(self.KL, in_axes=(0,0,0,0,0,0,0), out_axes=0)(v_mean, 
                                                                        Lambda_diag_blocks, 
                                                                        Lambda_offdiag_blocks,
                                                                        R_diag_blocks,
                                                                        R_offdiag_blocks,
                                                                        S_inv_diag_blocks,
                                                                        S_inv_offdiag_blocks))
    ...
    @partial(jit, static_argnums=(0,))
    def loss(self, params: dict, batch: "Dataset") -> f64:
        """Compute the negative ELBO given a batch of data."""

        elbo = self.compute_ELBO(params, batch)
    ...
    def fit(
        ...
        """Fits the FSDE with mini-batch SGD and/or NGD."""

        model_params = dict(W=self.W,
                            unc_var=self.unc_var,
                            unc_lengthscales=self.unc_lengthscales,
                            unc_k_vars=self.unc_k_vars)

        v_params = dict(v_mean=self.v_mean,
                        unc_R_diag_blocks=self.unc_R_diag_blocks,
                        unc_R_offdiag_blocks=self.unc_R_offdiag_blocks)

        # Initialize optimizers
        model_optimizer = optax.adam(lr['model_lr']) 
        opt_state = model_optimizer.init(model_params)
        ...
        v_opt_state = v_optimizer.init(v_params)

        # Collect initial setting
        init_state = (model_params, v_params, opt_state, v_opt_state)

        # Track total training time
        start_time = datetime.now()

        # Single optimization step
        @progress_bar_scan(n_steps, log_rate)
        def step(carry, current):
            # Unpack
            model_params, v_params, opt_state, v_opt_state = carry
            params = model_params | v_params
            ...
            # Compute gradients
            out, grads = value_and_grad(self.loss)(params, batch)
            elbo = -out

            # Update model parameters via ADAM
            model_grads = dict(W=grads['W'],
                               unc_var=grads['unc_var'],
                               unc_lengthscales=grads['unc_lengthscales'],
                               unc_k_vars=grads['unc_k_vars'])

            model_updates, opt_state = model_optimizer.update(model_grads, opt_state)
            model_params = optax.apply_updates(model_params, model_updates)

            # Update variational parameters via NGD
            def natgrad_update(v_params, v_opt_state):
                v_grads = (grads['v_mean'].astype(jnp.float64), 
                           grads['unc_R_diag_blocks'].astype(jnp.float64),
                           grads['unc_R_offdiag_blocks'].astype(jnp.float64))

                # Compute gradient wrt expectation parameters
                exp_params = self.params_to_exp(params['v_mean'].astype(jnp.float64),
                                                params['unc_R_diag_blocks'].astype(jnp.float64),
                                                params['unc_R_offdiag_blocks'].astype(jnp.float64))

                _, exp_vjp = vjp(self.exp_to_params, *exp_params)
                grads_exp = exp_vjp(v_grads)

                # Compute natural gradient wrt parameters
                nat_params = self.params_to_nat(params['v_mean'].astype(jnp.float64),
                                                params['unc_R_diag_blocks'].astype(jnp.float64),
                                                params['unc_R_offdiag_blocks'].astype(jnp.float64))

                _, natgrads = jvp(self.nat_to_params, nat_params, grads_exp)
                
                natgrad_dict = {'v_mean': natgrads[0],
                                'unc_R_diag_blocks': natgrads[1],
                                'unc_R_offdiag_blocks': natgrads[2]}
            ...
            # Update variational parameters via Adam
            def adam_update(v_params, v_opt_state):
                v_grads = dict(v_mean=grads['v_mean'],
                               unc_R_diag_blocks=grads['unc_R_diag_blocks'],
                               unc_R_offdiag_blocks=grads['unc_R_offdiag_blocks'])

                v_updates, v_opt_state = v_optimizer.update(v_grads, v_opt_state)
                v_params = optax.apply_updates(v_params, v_updates)
        ...
        (model_params, v_params, _, _), elbos = lax.scan(step, init_state, (step_idxs, keys))
        ...
        self.unc_var = model_params['unc_var']
        self.unc_lengthscales = model_params['unc_lengthscales']
        self.unc_k_vars = model_params['unc_k_vars']
        self.v_mean = v_params['v_mean']
        self.unc_R_diag_blocks = v_params['unc_R_diag_blocks']
        self.unc_R_offdiag_blocks = v_params['unc_R_offdiag_blocks']
    ...
    def past_moments(
        ...
        chol_infty_i = jscipy.linalg.cholesky(cov_infty_i + self.jitter * jnp.eye(self.D), lower=True) # [D,D]
        chol_factor_i = jscipy.linalg.solve_triangular(chol_infty_i, psi @ cov_infty_i, lower=True) # [D,D]
        factor_i = jscipy.linalg.solve_triangular(chol_infty_i.T, chol_factor_i, lower=False) # [D,D]
    ...
    def predict(
        ...
        # Compute approximate mean and covariance
        ax_time = (0,0,None,None,None,None,None)
        ax_latent = (None,None,0,0,0,0,0)
        args = [times, modes, cov_infty, F, v_mean, S_inv_diag_blocks, S_inv_offdiag_blocks]
        approx_mean, approx_cov = vmap(vmap(self.approx_moments, in_axes=ax_time), 
                                       in_axes=ax_latent)(*args)
